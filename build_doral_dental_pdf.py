"""
Generate a formatted PDF report: Doral FL Dental Offices — Low Online Presence Leads
"""
...
def make_styles():
    ...
    s["card_value"] = ParagraphStyle(
        "card_value", fontName="Helvetica", fontSize=9, textColor=TEXT_DARK,
    )
...
def add_page_decor(canvas, doc):
    ...
    canvas.drawCentredString(
        w / 2, 0.1 * inch,
        f"Doral FL Dental Office Lead Report  |  Page {doc.page}  |  Researched June 2026  |  Haven Media Inc.",
    )
    canvas.restoreState()
...
def social_badge_text(practice):
    ...
    return " · ".join(platforms) if platforms else "None detected"


def build_practice_card(p):
    ...
    contact_data = [
        ...
        [Paragraph("<b>Social Media</b>", ST["card_label"]), Paragraph(social, ST["card_value"]),
         Paragraph("<b>Platforms Active</b>", ST["card_label"]),
         Paragraph(f"{p.get('social_platforms_count', 0)}", ST["card_value"])],
    ]
    ...
    card_inner = Table(
        [
            [Paragraph(p["practice_name"], ST["card_name"])],
            [Paragraph(p["doctor_name"], ST["card_doctor"])],
            [Paragraph(f"<b>Specialty:</b> {p['specialty']}", ST["card_value"])],
            [Spacer(1, 3)],
            [contact_tbl],
            [Spacer(1, 3)],
            [Paragraph(f"<b>Website Assessment:</b> {p.get('website_quality_notes', '')}", ST["card_note"])],
            [Paragraph(f"<b>★ Opportunity:</b> {p.get('key_opportunity', '')}", ST["card_opp"])],
        ],
        colWidths=[6.5 * inch],
    )
...
def build_summary_table(practices):
    ...
    for i, p in enumerate(sorted_p, 1):
        c = TIER_COLOR[p["opportunity_tier"]]
        style += [
            ("TEXTCOLOR", (4, i), (4, i), c),
...
def build_pdf():
    output = "doral_dental_lead_report.pdf"
    doc = SimpleDocTemplate(
        ...
        title="Doral FL Dental Office Lead Report",
        author="Haven Media Inc.",
    )

    high_count = sum(1 for p in PRACTICES if p["opportunity_tier"] == "HIGH")
    med_count  = sum(1 for p in PRACTICES if p["opportunity_tier"] == "MEDIUM")
    low_count  = sum(1 for p in PRACTICES if p["opportunity_tier"] == "LOW")
    total      = len(PRACTICES)
    ...
    story.append(Paragraph("Dental Offices", ParagraphStyle(
        ...
        leading=40, textColor=WHITE, alignment=TA_CENTER, spaceAfter=14)))
    story.append(Paragraph("Low Social Media &amp; Web Presence", ParagraphStyle(
        "ct3", fontName="Helvetica-Bold", fontSize=20,
        leading=26, textColor=colors.HexColor("#B0C4DE"), alignment=TA_CENTER, spaceAfter=6)))
    ...
    story.append(Paragraph(
        "Researched: June 2026   |   Prepared by Haven Media Inc.",
    ...
    intro_blocks = [
        ("Opportunity Tiers", True,
         "Each dental practice is assigned a tier based on its total digital presence score (0–100). "
        ...
        (None, False,
         ...
         "No website or basic/outdated site. Best candidates for a complete digital build-out."),
        (None, False,
         "<b><font color='#E67E22'>MEDIUM (score 20–44):</font></b>  Facebook-only or 1–2 platforms "
         "with very low engagement. Strong candidates for social expansion, Instagram setup, and website upgrades."),
        (None, False,
         "<b><font color='#27AE60'>LOW (score 45+):</font></b>  Already established online. "
         "Included for competitive context. Lower priority for outreach."),
        ("Scoring Breakdown", True,
         "Website present (+15) · SSL/modern site (+5) · Facebook (+8) · Instagram (+8) · "
         "Twitter/X (+5) · LinkedIn (+4) · YouTube (+5) · TikTok (+5) · Contact form (+3) · Schema markup (+3)"),
        ("Why Dental Offices in Doral?", True,
         "Doral is a fast-growing Miami suburb with a predominantly Latin American population and strong disposable income. "
         "Dentistry is highly visual — smile makeovers, whitening, implants, and before/after transformations "
         "are among the highest-performing content categories on Instagram and TikTok. "
         "Many Doral dental practices still rely on referrals and walk-in traffic, "
         "leaving a wide-open digital channel for whoever moves first."),
    ]
    for heading, is_bold, body in intro_blocks:
        if heading:
            story.append(Paragraph(heading, ST["intro_bold"]))
        story.append(Paragraph(body, ST["intro_body"]))
    story.append(Spacer(1, 0.15 * inch))

    # ── SUMMARY TABLE ─────────────────────────────────────────────────────────
    story.append(Paragraph("All Practices — At a Glance", ParagraphStyle(
        "h2", fontName="Helvetica-Bold", fontSize=12,
        textColor=DARK_NAVY, spaceAfter=6)))
    story.append(build_summary_table(PRACTICES))
    story.append(PageBreak())

    # ── DETAILED CARDS BY TIER ────────────────────────────────────────────────
    for tier, label_text in [("HIGH", "HIGH Opportunity"), ("MEDIUM", "MEDIUM Opportunity"), ("LOW", "LOW Opportunity")]:
        tier_practices = [p for p in PRACTICES if p["opportunity_tier"] == tier]
        if not tier_practices:
            continue

        t_col = TIER_COLOR[tier]
        hdr_tbl = Table(
            [[Paragraph(f"{label_text}  ({len(tier_practices)} practices)", ST["section_header"])]],
            colWidths=[6.5 * inch],
        )
        ...
        for p in tier_practices:
            story.extend(build_practice_card(p))
